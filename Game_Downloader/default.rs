...
use iced::{
    ...
    tooltip::Position,
    Alignment, Command, Image, Length, ProgressBar, Text, Color, Background
};
...
impl DefaultView {
    ...
    pub fn view(&self, active_profile: &Profile) -> Element<DefaultViewMessage> {
        ...
        let title = Text::new("TOWNHALL")
        .size(40)
        .color(Color::WHITE);

        let bg = container(Image::new(Handle::from_memory(crate::assets::VOXELLAND_BACKGROUND.to_vec()))
        .width(Length::Fill)
        .height(Length::Fill)
    );
        ...
        // Contains logo, changelog and news .push(right)
        ...
        let mut background_v = container(
            column()
            .width(Length::Fill)
            .height(Length::Fill)
            .push(top)
    ...
    pub fn update(
        ...
        match msg {
            ...
            DefaultViewMessage::Interaction(interaction) => match interaction {
                Interaction::PlayPressed(game) =>   townhall::main(game),
                ...
                Interaction::ReadMore(mut new_url) => {
                    new_url = "pixelland.app".to_string();
                }
            },
        }
